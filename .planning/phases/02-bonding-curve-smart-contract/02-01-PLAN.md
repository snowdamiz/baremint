...
must_haves:
  ...
  artifacts:
    ...
    - path: "programs/baremint/src/state/bonding_curve.rs"
      provides: "BondingCurve account struct with virtual/real reserves and fee accrual fields"
...
<task type="auto">
  ...
  <action>
    ...
    4. Create all state account structs per RESEARCH.md specifications:
       ...
       - platform_fees_accrued: u64 (tracks platform's share of fees held in this PDA's lamports)
       - creator_fees_accrued: u64 (tracks creator's share of fees held in this PDA's lamports)
       - bump: u8
       - Use #[account] and #[derive(InitSpace)]
       ...
       - IMPORTANT: All SOL (reserves + accrued fees) is held in the bonding_curve PDA itself. The fee accrual fields track how much of the PDA's lamports belong to platform vs creator fees, so withdrawal instructions know what to send.
  ...
  <done>
    Anchor project compiles. All four state account structs (GlobalConfig, BondingCurve, VestingAccount, CreatorProfile) are defined with correct fields and InitSpace. BondingCurve includes platform_fees_accrued and creator_fees_accrued fields for the fee accrual pattern. Error codes cover all planned failure modes. TypeScript test dependencies installed.
  </done>
</task>
...
- BondingCurve struct includes platform_fees_accrued and creator_fees_accrued u64 fields
...
- BondingCurve has fee accrual fields (platform_fees_accrued, creator_fees_accrued) from day one

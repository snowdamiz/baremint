...
must_haves:
  truths:
    ...
    - "Buying tokens returns correct amount based on constant product formula with fees deducted"
    - "Selling tokens returns correct SOL based on constant product formula with fees deducted"
    - "Slippage protection reverts transactions when price moves beyond tolerance"
    - "Burn-for-access destroys correct token amount and extracts fees from reserves"
    - "Vesting cliff blocks claims before 30 days, allows weekly claims after"
    - "Fee withdrawal sends accrued fees to authority/creator"
  ...
  artifacts:
    ...
    - path: "tests/buy_sell.test.ts"
      provides: "Trading tests with curve math verification"
...
<task type="auto">
  ...
  <action>
    ...
    5. Create tests/create_token.test.ts:
       ...
       - Test: BondingCurve state has correct initial reserves
    ...
    6. Create tests/buy_sell.test.ts:
       ...
       - Test: buy deducts 5% fee (verify bonding_curve.platform_fees_accrued and creator_fees_accrued)
       ...
       - Test: sell deducts 5% fee from SOL output
  ...
  <done>
    Test infrastructure established with bankrun, TS math mirrors, and shared helpers. Initialize tests verify config setup and validation. Create_token tests verify supply distribution, mint revocation, and 90-day cooldown. Buy/sell tests verify curve math, fee deduction, slippage protection, and reserve tracking.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    1. Create tests/burn.test.ts:
       ...
       - Test: burn_for_access extracts 5% fee from curve reserves
    ...
    3. Create tests/fees.test.ts:
       ...
       - Test: withdraw_platform_fees sends correct SOL to authority
       - Test: withdraw_platform_fees resets accrued amount to 0
       - Test: withdraw_platform_fees fails for non-authority (Unauthorized)
       - Test: withdraw_creator_fees sends correct SOL to creator
       - Test: withdraw_creator_fees fails for non-creator (Unauthorized)
       - Test: fees accumulate correctly across multiple trades (buy, sell, buy, sell -> verify totals)
  ...
  <done>
    Burn tests verify deflationary mechanics, fee extraction, and SOL-denominated pricing. Vesting tests verify 30-day cliff, weekly windows, full vest, and revocation with time manipulation. Fee tests verify accumulation and withdrawal for both platform and creator. Program deployed to devnet with verified program ID. All phase success criteria met.
  </done>
</task>
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    2. Review test output for coverage of key scenarios:
       ...
       - Buy/sell with correct pricing and fees
       - Burn-for-access deflationary mechanics
...
- Phase 2 success criteria verified:
  1. SPL token created on devnet with bonding curve PDA holding SOL reserves
  2. Tokens bought/sold through curve with correct pricing and slippage protection
  3. Burning tokens for content access returns correct SOL amount from curve (actually: burns tokens, no SOL returned, deflationary)
  4. Creator vesting enforces 30-day cliff and 60-day linear vest
  5. Platform and creator fees collected on every trade
</verification>

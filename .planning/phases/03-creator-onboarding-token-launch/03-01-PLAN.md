...
user_setup:
  - service: cloudflare-r2
    why: "Image storage for creator avatars, banners, and token images"
    env_vars:
      - name: R2_ENDPOINT
      ...
      - name: R2_SECRET_ACCESS_KEY
        source: "Cloudflare Dashboard -> R2 -> Manage R2 API Tokens -> Create API token"
      - name: R2_PUBLIC_URL
        source: "Cloudflare Dashboard -> R2 -> Bucket -> Settings -> Public access (or custom domain)"
    dashboard_config:
      - task: "Create R2 bucket with public access enabled"
...
<objective>
Create the creator profile foundation: database schema for creator profiles and tokens, Cloudflare R2 image upload with presigned URLs, and the first steps of the onboarding wizard (profile info + avatar/banner upload with cropping).
...
2. Create lib/storage/upload.ts with R2 presigned URL generation using @aws-sdk/client-s3 and @aws-sdk/s3-request-presigner (R2 is S3-compatible). Export `generatePresignedUploadUrl(key: string, contentType: string): Promise<string>`. Configure S3Client with `endpoint: process.env.R2_ENDPOINT` (format: `https://<ACCOUNT_ID>.r2.cloudflarestorage.com`), `region: "auto"`, and credentials from R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY env vars. Bucket from R2_BUCKET. Use PutObjectCommand with 5MB max content length, 15-minute expiry. Key format: `creators/{userId}/{uuid}.{ext}`. Public URL pattern: `${R2_PUBLIC_URL}/${key}` (R2 public bucket URL or custom domain).
...
5. Add R2 env vars to .env.example:
```
...
R2_PUBLIC_URL=            # Public bucket URL or custom domain for serving images
```
...
3. Create components/creator/steps/profile-step.tsx:
   ...
   - Avatar upload button: opens file picker, then image-cropper dialog with circularCrop. On crop complete: upload to R2 via presigned URL (fetch POST /api/upload/presign, then PUT to returned uploadUrl), store publicUrl in state
...
5. Create app/(dashboard)/dashboard/creator/page.tsx:
  ...
  <verify>
    ...
    - Avatar upload opens cropper dialog, cropping produces a blob (R2 upload will fail without real R2 creds, but the flow should work up to the upload call)
...
- Presigned upload API generates valid R2 presigned URLs

...
@.planning/ROADMAP.md
@.planning/STATE.md
@.planning/phases/07-burn-to-unlock-premium-content/07-01-SUMMARY.md
...
interface UnlockDialogProps {
  ...
  mintAddress: string;       // NEW: needed for trade page link
  onUnlocked?: () => void;  // NEW: callback to refresh content after unlock
}
```

Use `useState` for dialog step: "quote" | "confirming" | "success" | "error".
Use `useEffect` to fetch burn quote when dialog opens and accessLevel is burn_gated.
...
<task type="auto">
  <name>Task 2: Post composer and post feed updates for burn_gated</name>
  <files>
    components/content/post-composer.tsx
    components/content/post-feed.tsx
  </files>
...
**2. Update post-feed.tsx (or wherever UnlockDialog is rendered):**

Pass the new required props to UnlockDialog:
- `postId`: from the post data
- `mintAddress`: look up from creatorToken data (may need to fetch or pass through)
- `onUnlocked`: callback that re-fetches the post's media data to show unlocked content

Find where UnlockDialog is rendered in the post feed / locked post component. Add the new props. If `mintAddress` is not available in the post feed context, fetch it from the creatorToken when the unlock dialog opens, or pass it through from the gated media API response.

If the post feed doesn't directly render UnlockDialog but delegates to a child component (like a locked overlay), trace the component tree and update accordingly.

The `onUnlocked` callback should invalidate/re-fetch the gated media data for that specific post so the content appears immediately without page refresh. Use the existing pattern of fetching `/api/content/${postId}/media` that the post feed already uses for lazy gated media loading.
  </action>
  <verify>
Run `npx tsc --noEmit` -- no TypeScript errors.
Verify burn_gated no longer shows tokenThreshold input in the composer.
Verify UnlockDialog receives postId and mintAddress props where rendered.
Verify onUnlocked callback triggers media re-fetch.
  </verify>
  <done>
Burn-gated access level in composer hides threshold input and shows informational text.
UnlockDialog receives all required props for burn execution.
Successful burn triggers content refresh showing unlocked media.
  </done>
</task>
